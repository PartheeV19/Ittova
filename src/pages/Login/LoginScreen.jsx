...
export default function LoginScreen() {
  const { currentView, setCurrentView, requestOtp, verifyOtp } = useApp();
      ...
      <section className="auth-terminal-card" aria-labelledby="sign-in-title">
        ...
        <div className="role-tabs-bar" aria-label="Choose a portal">
          {[['customer', 'Customer'], ['vendor', 'Vendor']].map(([view, label]) => (
            <button type="button" key={view} disabled={busy} aria-pressed={role === (view === 'vendor' ? 'supplier' : view)} className={`role-tab-btn ${role === (view === 'vendor' ? 'supplier' : view) ? 'active' : ''}`} onClick={() => { setCurrentView(view); setSent(false); setCode(''); setError(''); }}>{label}</button>
          ))}
        </div>
