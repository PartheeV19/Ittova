...
export default function StartingGate({ onComplete }) {
  ...
  const { requestOtp, requestSharedOtp, verifyOtp } = useApp();
  ...
  const handleSubmit = async (e) => {
    ...
    if (formData.phone.trim() && !/^\+[1-9][0-9]{7,14}$/.test(formData.phone.replace(/[\s().-]/g, ''))) newErrors.phone = 'Include your country code, for example +919876543210';
    ...
    const phone = formData.phone.trim() ? formData.phone.replace(/[\s().-]/g, '') : '';
    ...
    try {
      if (!codeComplete) {
        // SMS delivery (MSG91/DLT) isn't live yet -- fall back to email-only
        // rather than block signup entirely when no phone was given.
        const result = phone ? await requestSharedOtp(email, phone) : await requestOtp(email, 'signup');
          ...
          <div className="gate-form-col">
            <div className="gate-form-card">
              ...
              <form onSubmit={handleSubmit} className="gate-form-body" aria-busy={busy}>
                ...
                <div className="grid-2" style={{ gap: '12px' }}>
                  ...
                  <div className="form-group-block">
                    <label htmlFor="gate-phone">
                      Mobile / Phone <span style={{ color: 'var(--muted)', fontWeight: 400 }}>(optional for now)</span>
                    </label>
                    <input
                      id="gate-phone"
                ...
                <p style={{ fontSize: '0.75rem', color: 'var(--muted)', margin: '8px 0 0' }}>You will receive the OTP by email{formData.phone.trim() ? ' and SMS, if configured' : ''}.</p>
